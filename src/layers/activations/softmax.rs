...
impl ActivationLayer for SoftMax {
    fn function(inputs: &Vec<f64>) -> Vec<f64> {
        let exponentials: &Vec<f64> = &inputs.from_powf(E);
    ...
    fn differential_of_output_with_respect_to_input(
        ...
        if input_index == output_index {
            self.last_outputs[sample_index][input_index] * (1.0 - self.last_outputs[sample_index][output_index])
        } else {
            -self.last_outputs[sample_index][input_index] * self.last_outputs[sample_index][output_index]
        }
    }
