kernel void compute_loss(
    ...
    for (int output_index = 0; output_index < outputs_amount; output_index++) {
        ...
        sample_loss -= expected_output * log(output + 0.00000001f) 
                            + (1.0f - expected_output) * log(1.0f - output + 0.00000001f);
    }
...
kernel void compute_loss_to_output_derivatives(
    ...
    loss_to_output_derivatives[flat_i] = -(expected_output / (output + 0.00000001f) 
                                - (1.0f - expected_output) / (1.0f - output + 0.00000001f));
}