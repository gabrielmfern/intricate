    ...
    fn should_compute_derivatives_up_to_a_certain_precision() {
        ...
        let expected_derivatives: Vec<f32> = expected_outputs
            ...
            .map(|(expected_output, actual_output)| {
                -(expected_output / (actual_output + 0.00000000000001)
                    - (1.0 - expected_output) / (1.0 - actual_output + 0.00000000000001))
            })
            .collect();
    ...
    fn should_compute_loss_up_to_a_certain_precision() {
        ...
        let expected_loss: f32 = expected_outputs
            ...
            .map(|(expected_output, output)| {
                -(expected_output * (output + 0.0000000000000001).ln()
                    + (1.0 - expected_output) * (1.0 - output + 0.0000000000000001).ln())
            })
            .sum::<f32>()
