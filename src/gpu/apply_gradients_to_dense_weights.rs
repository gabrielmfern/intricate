...
async fn execute_gpu_code(
    ...
    inputs_amount: usize,
    flattened_layer_inputs: &[f64],
    flattened_layer_output_to_error_derivatives: &[f64],
    flattened_layer_weights: &[f64]
) -> Option<Vec<Vec<f64>>> {
    ...
    let weights_size = inputs_amount * outputs_amount * std::mem::size_of::<f64>();
    ...
    let learning_rate_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        ...
        contents: &learning_rate.to_ne_bytes(),
    ...
    let inputs_amount_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        ...
        contents: &inputs_amount.to_ne_bytes(),
        usage: wgpu::BufferUsages::UNIFORM
            | wgpu::BufferUsages::COPY_DST
            | wgpu::BufferUsages::COPY_SRC,
    });
    ...
    let outputs_amount_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        ...
        contents: &outputs_amount.to_ne_bytes(),
        usage: wgpu::BufferUsages::UNIFORM
            | wgpu::BufferUsages::COPY_DST
            | wgpu::BufferUsages::COPY_SRC,
    });
    ...
    let samples_amount_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        ...
        contents: &samples_amount.to_ne_bytes(),
        usage: wgpu::BufferUsages::UNIFORM
            | wgpu::BufferUsages::COPY_DST
            | wgpu::BufferUsages::COPY_SRC,
    });
    ...
    let samples_amount_float_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        ...
        contents: &(samples_amount as f64).to_ne_bytes(),
        usage: wgpu::BufferUsages::UNIFORM
            | wgpu::BufferUsages::COPY_DST
            | wgpu::BufferUsages::COPY_SRC,
    });
    ...
    if let Some(Ok(())) = receiver.receive().await {
        ...
        let flattened_new_weights: Vec<f64> = result.to_vec();
        ...
        let new_weights: Vec<Vec<f64>> = (0..inputs_amount)
            ...
            .collect();
        // println!(
        //     "{}s elapsed unflattening calculated gradients",
        //     unflatten_start.elapsed().as_secs_f64()
        // );
