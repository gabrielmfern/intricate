    ...
    fn compute_update_vectors(
            ...
            .get_mut(&(layer_index, parameter_id.to_string()))
        {
            if last_update_vector.size()? != gradients_size {
                return Err(OptimizationError::InvalidParametersSize(
                    format!(
                        "The last update vector in the Nesterov optimizer does not match the gradients received when trying to compute a new update vector for the paramter with id '{}'", 
